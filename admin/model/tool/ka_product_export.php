	...
	*/
	public function process() {
		...
		if ($from == 0) {
			...
			if ($this->use_escape) {
				$put_res = fputcsv($handle, $row, $this->params['delimiter'], $this->enclosure, "\x00");
			} else {
				$put_res = fputcsv($handle, $row, $this->params['delimiter'], $this->enclosure);
			}
			...
			$query_orderby = " GROUP BY p.product_id ORDER BY p.model, p.product_id ";
