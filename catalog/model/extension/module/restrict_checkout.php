...
class ModelModuleRestrictCheckout extends Model {
	...
	public function restrict() {
		...
		foreach ($settings['restriction'] as $row) {
			...
			if (isset($rules['product_group'])) {
				$row_disabled_text = '"' . $row['name'] . '" ignored';
			...
			if (empty($product_keys) && empty($this->session->data['vouchers'])) {
				if ($this->row['testing_mode']) {
					$this->log->write(strtoupper($this->name) . ': "' . $row['name'] . '" ignored for having no eligible products');
				}
				continue;
			}
	...
	private function ruleViolation($rule, $value) {
		...
		if ($this->row['testing_mode'] && $violation) {
			$this->log->write(strtoupper($this->name) . ': "' . $this->row['name'] . '" ignored for violating rule "' . $rule . ' ' . $comparison . ' ' . implode(', ', $rules[$rule][$comparison]) . '" with value "' . (is_array($value) ? implode(',', $value) : $value) . '"');
		}
	...
	private function inRange($value, $range_list, $type = '') {
		...
		if ($this->row['testing_mode'] && (strpos($type, ' not') ? $in_range : !$in_range)) {
			$this->log->write(strtoupper($this->name) . ': "' . $this->row['name'] . '" ignored for violating rule "' . $type . (strpos($type, ' not') ? ' is not ' : ' is ') . implode(', ', $range_list) . '" with value "' . $value . '"');
		}
