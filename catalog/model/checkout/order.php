	...
	/*public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		$order_info = $this->getOrder($order_id);
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
                        ...
                        //Check if credit purchase order
                        $order_credit_query = $this->db->query("SELECT * from `".DB_PREFIX ."order_credit` WHERE order_id = '" . (int)$order_id . "'");
                        //print_r($order_credit_query);
                        if($order_credit_query->num_rows > 0){
                            $order_info['credit_purchase'] = 1;
                        }else{
                            $order_info['credit_purchase'] = 0;
                        }
				...
				$order_total_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_total` WHERE order_id = '" . (int)$order_id . "' ORDER BY sort_order ASC");
                                ...
                                foreach ($order_total_query->rows as $order_total) {
                                    ...
                                    if($order_total['code'] != 'speedy_delivery_fee') {
        ...
        public function updateGuestOrder($data){
	   $this->db->query("UPDATE `" . DB_PREFIX . "order` SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "' where order_id = '" . $this->db->escape($data['order_id']) . "' ");
        }
        
        //Custom Added : To edit order status if voided in vend
        public function editOrderForVend($order_id,$data){
