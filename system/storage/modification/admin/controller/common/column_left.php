...
///karapuz (ka_csv_product_export.ocmod.xml) 
        ...
        if ($this->user->hasPermission('access', 'report/customer_buy_credit')) {
    		if ($this->user->hasPermission('access', 'report/customer_buy_credit')) {
    			$report_customer[] = array(
    				'name'	   => $this->language->get('text_report_customer_buy_credit'),
    				'href'     => $this->url->link('report/customer_buy_credit', 'token=' . $this->session->data['token'], true),
    				'children' => array()
    			);
    		}
        }
