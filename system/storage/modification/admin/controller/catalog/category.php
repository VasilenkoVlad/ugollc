...
$this->load->config('dragndrop_position');
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'name'        => $result['name'],
				'sort_order'  => (!$this->config->get($this->config->get('ddp_module_name') . '_status')) ? $result['sort_order'] : $this->db->query("SELECT sort_order FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$result['category_id'] . "'")->row['sort_order'],
