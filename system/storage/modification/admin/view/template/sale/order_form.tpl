          ...
          <div class="tab-content">
            ...
            <tr>
              <td class="text-left"><?php echo $order_credit['description']; ?>
                ...
                <input type="hidden" name="credit[<?php echo $credit_row; ?>][customer_id]" value="<?php echo $order_credit['customer_id']; ?>" />
                <input type="hidden" name="credit[<?php echo $credit_row; ?>][description]" value="<?php echo $order_credit['description']; ?>" />
                <input type="hidden" name="credit[<?php echo $credit_row; ?>][firstname]" value="<?php echo $order_credit['firstname']; ?>" />
                <input type="hidden" name="credit[<?php echo $credit_row; ?>][lastname]" value="<?php echo $order_credit['lastname']; ?>" />
                <input type="hidden" name="credit[<?php echo $credit_row; ?>][email]" value="<?php echo $order_credit['email']; ?>" />
            ...
            <div class="tab-pane" id="tab-credit">
              <fieldset>
                ...
                <div class="form-group required">
                  ...
                  <div class="col-sm-10">
                    <input type="text" name="amount-credit" value="" id="input-amount-credit" class="form-control" />
                  </div>
                </div>
              </fieldset>
              <div class="text-right">
...
$('#button-refresh').on('click', function() {
	$.ajax({
		...
		success: function(json) {
			...
			if (json['credits'].length) {
				for (i in json['credits']) {
					credit = json['credits'][i];

					html += '<tr>';
					html += '  <td class="text-left">' + credit['description'];
                    html += '    <input type="hidden" name="credit[' + i + '][description]" value="' + credit['description'] + '" />';
                    html += '    <input type="hidden" name="credit[' + i + '][customer_id]" value="' + credit['customer_id'] + '" />';
                    html += '    <input type="hidden" name="credit[' + i + '][firstname]" value="' + credit['firstname'] + '" />';
                    html += '    <input type="hidden" name="credit[' + i + '][lastname]" value="' + credit['lastname'] + '" />';
                    html += '    <input type="hidden" name="credit[' + i + '][email]" value="' + credit['email'] + '" />';
                    html += '    <input type="hidden" name="credit[' + i + '][amount]" value="' + credit['amount'] + '" />';
					html += '  </td>';
					html += '  <td class="text-left"></td>';
					html += '  <td class="text-right">1</td>';
					html += '  <td class="text-right">' + credit['price'] + '</td>';
					html += '  <td class="text-right">' + credit['price'] + '</td>';
            ...
            if (!json['products'].length && !json['vouchers'].length && !json['credits'].length) {
            ...
            if (!json['totals'].length && !json['products'].length && !json['vouchers'].length && !json['credits'].length) { 
...
$('#button-customer').on('click', function() {
	$.ajax({
		...
		success: function(json) {
            ...
            var request_3 = request_1.then(function() {
                $.ajax({
                    url: '<?php echo $catalog; ?>index.php?route=api/buy_credit/add&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
                    type: 'post',
