		...
		if (!empty($this->session->data['vouchers'])) {
    		...
    		if (!empty($this->session->data['credits'])) {
    			foreach ($this->session->data['credits'] as $credit) {
    				$item_total += $this->currency->format($credit['amount'], $this->session->data['currency'], false, false);
    
    				$data['L_PAYMENTREQUEST_0_DESC' . $i] = '';
    				$data['L_PAYMENTREQUEST_0_NAME' . $i] = $credit['description'];
    				$data['L_PAYMENTREQUEST_0_NUMBER' . $i] = 'CREDIT';
    				$data['L_PAYMENTREQUEST_0_QTY' . $i] = 1;
    				$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($credit['amount'], $this->session->data['currency'], false, false);
    				$i++;
    			}
    		}
